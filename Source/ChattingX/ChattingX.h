...
class ChattingXFunctionLibrary
{
public:
	static void MyPrintString(const AActor* InWorldContextActor, const FString& InString, float InTimetoDiplay = 1.f, FColor InColor = FColor::Cyan)
	{
		if (IsValid(GEngine) == true && IsValid(InWorldContextActor) == true)
		{
			...
			{
				GEngine->AddOnScreenDebugMessage(-1, InTimetoDiplay, InColor, InString);
			}
			else
			{
